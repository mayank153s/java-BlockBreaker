...
public class BlockBreakerPanel extends JPanel implements KeyListener {
	...
	int size = 25;//25
	BlockBreakerPanel(){
		paddle = new Block(175,400,150,25,"pad.png");
		for(int i=0;i<8;i++){
		...
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),50,60,25,"green 1.png"));
		}
		for(int i=0;i<8;i++){
		...
		ball.add(new Block(237,390,25,15,"ball.png"));//237,437,25,25    //237,390,25,15
		addKeyListener(this);
		setFocusable(true);
	}
	...
	public void update(){
		for(Block p : powerup){
			...
			if(p.intersects(paddle) && !p.destroyed){
				p.destroyed = true;
				ball.add(new Block(paddle.dx,390,25,15,"ball.png"));
			}
		}
		...
		for(Block ba : ball){
			...
			for(Block b: blocks){
				if((b.left.intersects(ba) || b.right.intersects(ba)) && !b.destroyed){
					...
					b.destroyed = true;
					if(b.powerup){
						powerup.add(new Block(b.x,b.y,25,19,"power.png"));
					}
				}
